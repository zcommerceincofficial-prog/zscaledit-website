/* ZScaledIt — fixed stone-graph backdrop + grand-opening intro
   A faceted-crystal ascending "scaling graph," fixed behind the whole
   page. Plays a quick draw-in animation behind an ink curtain on every
   full page load/refresh, then settles into place as the permanent
   backdrop. Respects prefers-reduced-motion (shows final state instantly). */
(function () {
  const STONE_SVG = `
<svg viewBox="0 0 1180 720" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  requestAnimationFrame(() => {
    bg.classList.add('is-in');
    setTimeout(() => curtain.classList.add('is-out'), 420);
    setTimeout(() => curtain.remove(), 780);
  });
})();
